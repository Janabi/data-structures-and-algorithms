...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(star => {
    let sectionTemplate = $('#template').first().clone();
    sectionTemplate.find('h2').text(star.name);
    sectionTemplate.find('h3').text(star.height);
    sectionTemplate.find('p').text(star['eye_color']);

    $('main').append(sectionTemplate);
  });
}
...
const stepActions = (recipe) => {
  let result = [];
  // Solution code here...
  let stepAction = recipe.steps;
  stepAction.forEach(value => {
      let list = value.split(" ").splice(0, 1);
      result.push(list.join(" "));
  });
  return result;
};
...
const removeVowels = (str) => {
  ...
  str1.forEach(value => {
      if(vowels.includes(value)) {
          str1.splice(str1.indexOf(value), 1);
      }
  });
...
const extractVowels = (str) => {
  ...
  arr.push(str1.join(""), vowels.join(""));
  return arr;
};
