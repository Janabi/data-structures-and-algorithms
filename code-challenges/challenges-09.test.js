...
const mapCurrentEvents = () => {
  // Solution code here...
  let eventObj = currentEvents.news.map(data => {
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let counter = arr.reduce((acc, value) =>{
    acc++;
    return acc;
  },0);
  return counter;
...
const reversedString = (str) => {
  // Solution code here...
  let arr = str.split('');
  let reversedString = arr.reduce((acc,val, idx)=>{
    acc[idx] = arr[arr.length - idx - 1];
    if (idx === arr.length) {
      acc[idx+1] = arr[0];
    }
    return acc;
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let numOfChildren = arr.reduce((acc, val, idx) =>{
...
const calculateAverage = (arr) => {
  // Solution code here...
  let average = arr.reduce((acc,val,idx) => {
    acc += val;
    if(idx === arr.length -1) {
      acc /= (idx+1);
    }
...
const extractStat = (statName, arr) => {
  // Solution code here...
  let statObj = arr.reduce((acc,val) =>{
...
const extractChildren = (arr) => {
  // Solution code here...
  let filterCharacter = arr.filter(value =>{
    let regex = /[a]/g;
    if(value.name.match(regex)) {
      return value;
    }
  });
  let childName = filterCharacter.reduce((acc,val) =>{
    if(val.children !== undefined) {
