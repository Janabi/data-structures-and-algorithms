...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.status(200).json("Hello Everyone!");
  });

  app.get('/aboutme', (request, response) => {
    response.status(200).send("I am Abdulrahman AL-Janabi, a chemical engineer who is seeking to switch his career and start with coding");
  });

  app.get('/favoritefoods', (request, response) => {
    response.status(200).send(['Dolma', 'Mansaf', 'Beef Burger', 'Sushi'])
  });

  app.get('*', (request, response) => {
    response.status(404).send("Opps! This page is not exist!");
  })
...
const count = (target, input) => {
  ...
  let numOfTimes = 0;
  input.forEach(value=>{
    numOfTimes += value.reduce((acc,val)=>{
      if (val === target) {
        acc++;
      }
      return acc;
...
const totalSum = (input) => {
  ...
  let sum = 0;
  input.forEach(value => {
    let eachArr = value.reduce((acc,val) => {
      acc+=val;
      return acc;
    }, 0);
    sum += eachArr;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let arr = [];
  input.forEach(value =>{
    let divisiblePow = value.reduce((acc,val) =>{
      if (val % 5 === 0 && typeof val !== 'string') {
        acc.push(Math.pow(2, val));
      }
      return acc;
    },[]);
    arr.push(divisiblePow);
  });
  return arr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let names = data.reduce((acc,val, idx)=>{
...
let findShortest = (data) => {
  // Solution code here...
  let heightCharacter = data.reduce((acc,val)=>{
    acc.push(val.height);
    return acc;
  }, []);
  let sortedHeight = heightCharacter.sort((a,b)=>{
    if(a < b) {
      return 1;
    } else if(a > b) {
      return -1;
  ...
  let filterShortest = data.filter(value=>{
    if (value.height === sortedHeight[0]) {
      return value.name;
    }
  });
  return filterShortest[0].name;
};
