...
const findHappiness = (arr) => {
  // Solution code here...
  let happiness = arr.filter(value=>{
    if(value.includes(':)')) {
      return true;
    }
  })
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let splitarr = arr.map(value=>{
    let realNum = value.split(' ').join('').split('(').join('').split(')').join('').split('-').join('');
    return realNum;
  })
  return splitarr;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let arr = str.split('');
  let arrStr = arr.filter(value=>{
    if (value.charCodeAt() > 47 && value.charCodeAt() < 58) {
...
const characterByIndex = (arr) => {
  // Solution code here...
  let arrChar = arr.map((value, index)=>{
    return value[index];
  });
  return arrChar;
};
