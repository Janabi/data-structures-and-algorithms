...
const templatingWithMustache = () => {
  // Solution code here...
  let arr =[];
  // let characterTemplate = $("template").html();
  // let showHtml = Mustache.render(characterTemplate, this);
  characters.forEach(value => {
    let characterTemplate = $("#template").html();
    let showHtml = Mustache.render(characterTemplate, value);
    arr.push(showHtml);
  });
  return arr;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((value, index)=> {
    if (value.name === character) {
      let characterValues = Object.values(value);
      if (characterValues[2].length === 0) {
        bol = false;
      } else {
        bol = true;
      }
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(value=> {
    total++;
    let characterValues = Object.values(value);
    if (characterValues[1] !== null) {
        total++;
    }
    if (characterValues[2].length !== 0) {
        total += characterValues[2].length;
    }
  });
  return total;
};
...
arr.forEach(value => {
   ...
   let member = 1;
   if (propertyValues[1] !== null) {
       member++;
   }
   if (propertyValues[2].length !== 0) {
       member += propertyValues[2].length;
   }
   let houseObj = new Object();
   houseObj.house = houseName;
   houseObj.members = member;
   sizes.push(houseObj);
});
return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(value => {
    ...
    let propertyValues = Object.values(value);
    let member = 1;
    if (propertyValues[1] !== null && !deceasedSpouses.includes(propertyValues[1])) {
        member++;
    }
    if (propertyValues[2].length !== 0) {
        member += propertyValues[2].length;
